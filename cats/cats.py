...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    i_m, diff_m = 0, limit+1
    for i in range(len(word_list)):
        if typed_word == word_list[i]:
            return word_list[i]
        else:
            diff_t = diff_function(typed_word, word_list[i], limit)
            if  diff_t < diff_m:
                i_m, diff_m = i, diff_t
    if diff_m > limit:
        return typed_word
    else:
        return word_list[i_m]
...
def minimum_mewtations(typed, source, limit):
    ...
    if limit < 0:
        return float('inf')
    if  not len(typed): # Base cases should go here, you may add more base cases as needed.
        # BEGIN
        "*** YOUR CODE HERE ***"
        return len(source)
        # END
...
def report_progress(typed, source, user_id, upload):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    typed_list, source_list = typed, source
    count = 0
    for i in range(len(typed_list)):
        if typed_list[i] == source_list[i]:
            ...
            break
    progress = count / len(source_list)
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    times = [[play_times[j + 1] - play_times[j] for j in range(len(play_times) - 1)] for play_times in tpp]
    words = words[:len(tpp[0]) -1]
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    k = [[] for _ in player_indices]
    for i in word_indices:
        j_min, t_min = 0, get_time(times, 0, i)
        for j in player_indices:
            t = get_time(times, j, i)
            if t < t_min:
                j_min = j
                t_min = t
        k[j_min].append(words[i])
    return k
